...
const std::unordered_map<char, unsigned short> nums = {
    {'I', 1},
    {'V', 5},
    {'X', 10},
    {'L', 50},
...
public:
    int romanToInt(string s) {
        ...
        for(unsigned short i = 0; i < s.length(); i++){
            unsigned short curr = nums.at(s[i]);
            if(i + 1 < s.length() && nums.at(s[i + 1]) > curr){
                integer += (nums.at(s[i + 1]) - curr);
                i++;
            } else {
                integer += curr;
            }
